...
{
public:
    OrderbookLevelInfo(const LevelInfos &bids,
...
{
public:
    Trade(const TradeInfo &bidTrade,
...
private:
    ...
    Trades MatchOrders()
    {
        ...
        // Cancel remaining FillAndKill BUY order
        if (!bids_.empty())
        {
        ...
        // Cancel remaining FillAndKill SELL order
        if (!asks_.empty())
        {
...
public:
    ...
    Trades ModifyOrder(OrderModify order)
    {
        ...
        const auto &[exixtingOrder, _] = orders_.at(oredr.GetOredrId());

        cancelOrder(order.GetOrderId());
        return AddOrder(order.ToOredrPointer(existingOrder->GetOrderType()));
    }
    ...
    OrderbookLevelInfo GetOrderbookLevelInfo() const
    {
        ...
        for (const auto &[price, oredrs] : bids_)
        {
            bidInfos.push_back(CreateLevelInfos(price, orders));
        }
        for (const auto &[price, oredrs] : asks_)
        {
...
int main()
{
    ...
    orderBook.AddOrder(std::make_shared<Order>(OrderType::GoodTillCancel, orderId + 1, Side::Sell, 90, 5));
